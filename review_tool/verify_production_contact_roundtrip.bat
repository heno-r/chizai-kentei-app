...
where uv >nul 2>nul
if %errorlevel%==0 (
  call uv run python verify_production_contact_roundtrip.py %*
  goto :end
)
...
if exist "%BUNDLED_PYTHON%" (
  ...
  "%BUNDLED_PYTHON%" verify_production_contact_roundtrip.py %*
  goto :end
)

echo Failed to run the production contact roundtrip check.
