...
function renderHistoryInsights(recentWeakest) {
    ...
    if (sessions.length === 0) {
        historyInsights.replaceChildren(createInfoCard("最初の1回", "履歴はまだありません", "まず1回解くと、最近の学習量や正答率の変化がここに表示されます。"), createInfoCard("続け方の目安", "短くても記録は残ります", "途中で中止した回も履歴に残るので、まずは短いセットから始めても大丈夫です。"));
        return;
    }
    const now = Date.now();
    ...
    const recentTrendCopy = previousThree.length === 0
    ...
    const cards = [
        createInfoCard("最近7日", `${weeklyAnswers}問 / ${last7DaysSessions.length}回`, weeklyAnswers > 0
            ? "この1週間でどれだけ触れたかをまとめています。短くても回数を重ねると履歴が安定してきます。"
            : "この1週間はまだ履歴がありません。まずは1セット解くと、ここからペースが見えやすくなります。"),
        createInfoCard("直近3回", `${recentRate}% 正解`, recentTrendCopy),
        createInfoCard("最後の学習", formatLastStudiedLabel(lastSession?.playedAt), lastSession
            ? `${MODE_LABELS[lastSession.mode] || lastSession.mode} を ${lastSession.answeredCount}問進めました。`
            : "まだ履歴がありません。"),
        createInfoCard("次に見直す", recentWeakest === "まだなし" ? "まずは基本問題を一巡" : recentWeakest, recentWeakest === "まだなし"
...
function renderPremiumValuePanel() {
    ...
    if (state.currentPlan === "premium") {
        ...
        premiumValueGrid.replaceChildren(createInfoCard("どこから復習するか迷う", "今日のおすすめと要復習キューで次を決める", "履歴と要復習をもとに、今やるとよいモードが見えるので、次の1セットを決めやすくなります。"), createInfoCard("苦手をまとめて潰したい", "カテゴリ別学習と間違えた問題だけ復習を使う", "著作権や商標など、崩れやすい分野と一度間違えた問題だけを短く回しやすくなります。"), createInfoCard("本番前に何をやるか決めたい", "直前14日モードで5問ずつ回す", "まずは5問ずつ。直前期は、少ない問数で間違えやすい論点を反復しやすくなります。"));
        return;
    }
    ...
    premiumValueGrid.replaceChildren(createInfoCard("どこから復習するか迷う", "今日のおすすめと要復習キューが増える", "履歴や自分で付けた要復習をもとに、次に解くべき問題を絞り込みやすくなります。"), createInfoCard("苦手をまとめて潰したい", "カテゴリ別学習と間違えた問題だけ復習が増える", "一度間違えた問題や弱い分野だけを回せるので、同じところで止まりにくくなります。"), createInfoCard("本番前に何をやるか決めたい", "直前14日モードで5問ずつ回せる", "試験が近い時期に、まずは少ない問数から直前向けの反復を始めやすくなります。"));
}
...
function renderPurchaseFollowup() {
    ...
    const purchaseState = state.authStatus.purchaseState || "not_started";
    const shouldShow = premiumActive ||
    ...
    if (returnedFromCheckout && premiumActive) {
        ...
        purchaseFollowupTips.replaceChildren(createInfoCard("1つ目", "今日のおすすめから始める", "履歴や要復習をもとに、今やるとよい学習モードをそのまま選べます。"), createInfoCard("2つ目", "カテゴリ別学習で苦手を絞る", "著作権や商標など、気になる分野だけを先に回したいときに向いています。"), createInfoCard("3つ目", "直前14日モードで仕上げる", "まずは5問ずつ。直前期は、少ない問数で間違えやすい問題を反復すると進めやすくなります。"));
        return;
    }
    if (premiumActive) {
        ...
        purchaseFollowupTertiary.classList.remove("hidden");
        purchaseFollowupTertiary.onclick = () => setActiveMode("final14", { startImmediately: true });
        purchaseFollowupTips.replaceChildren(createInfoCard("1つ目", "おすすめを確認する", "まずは今の履歴に合ったモードを見て、どこから進めるかを決めやすくします。"), createInfoCard("2つ目", "要復習に印を付ける", "迷った問題に印を付けておくと、あとで要復習だけをまとめて回せます。"), createInfoCard("3つ目", "直前14日モードに切り替える", "試験が近づいたら、まずは5問ずつ反復できる直前14日モードが使いやすくなります。"));
        return;
    }
    if (purchaseState === "refunded" || purchaseState === "revoked" || purchaseState === "expired") {
        ...
        purchaseFollowupTips.replaceChildren(createInfoCard("現在", "無料版だけ使える状態です", "解き直しや履歴確認はそのまま続けられます。プレミアム版の専用機能は表示されません。"), createInfoCard("確認しやすい点", "返金や利用停止のあとも状態が残ります", "ログイン後にこの案内が見えるので、現在の状態を画面で確認しやすくしています。"), createInfoCard("必要になったら", "内容を見直してから進めます", "もう一度使いたくなったときは、内容を見直してから購入前の確認へ進めます。"));
        return;
    }
    if (purchaseState === "cancelled") {
        ...
        purchaseFollowupTips.replaceChildren(createInfoCard("現在", "無料版のまま続けられます", "学習履歴や無料問題はそのまま残るので、必要ならここから続けられます。"), createInfoCard("次に進むとき", "内容確認からやり直せます", "価格や使える機能を見直してから、改めて購入前の確認へ進めます。"), createInfoCard("気にしなくてよい点", "もう一度ログインし直す必要はありません", "ログイン済みなら、そのまま購入前の確認へ戻って再開できます。"));
        return;
    }
    if (returnedFromCheckout && signedIn) {
        ...
        purchaseFollowupTips.replaceChildren(createInfoCard("確認中", "反映に少し時間がかかることがあります", "購入直後は、決済完了からプレミアム版の反映まで短い待ち時間が出ることがあります。"), createInfoCard("この間にできること", "無料版はそのまま続けられます", "反映待ちの間も、無料版の問題や復習履歴の確認はそのまま続けられます。"), createInfoCard("変化の目安", "プレミアム版の表示に切り替わります", "反映が完了すると、この画面にプレミアム版の使い方が表示されます。"));
        return;
    }
...
function renderStudyPlan() {
    ...
    for (let day = 1; day <= 14; day += 1) {
        ...
        cards.push(createInfoCard(`Day ${day}`, category, `${intensity}で 5問前後。${day % 3 === 0 ? "前日に迷った論点も再確認。" : "まずは今日の分だけで大丈夫です。"}`, "plan-card"));
    }
...
function renderRecommendation() {
    ...
    if (recommendDetails) {
        const cards = [
            ...
            createInfoCard(recommendation.reasonLabel, recommendation.reasonCopy, `モード: ${MODE_LABELS[recommendation.mode] || "順番に学習"}`),
        ];
