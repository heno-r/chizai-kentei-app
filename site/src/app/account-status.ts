(function attachAppAccountStatus(): void {
  ...
  function buildSignedInFreeMessage(status: LicenseStatusResponse): {
    title: string;
    copy: string;
    upgradeHref: string;
    upgradeLabel: string;
  ...
  async function refresh(): Promise<void> {
    if (!authClient) {
      renderMessage(
        "ログインの準備中です",
        "いまは無料版をそのまま試せます。ログイン後の購入状況の確認は、時間をおいてもう一度お試しください。",
        false,
        {
    ...
    try {
      ...
      if (!status.signed_in) {
        renderMessage(
          isPurchaseEnabled() ? "次はログインして進めます" : "今は無料版と内容確認を進められます",
          isPurchaseEnabled()
      ...
      renderMessage(
        "購入状況を確認できませんでした",
        "通信の状況によって購入状況の確認に失敗することがあります。しばらくしてからもう一度お試しください。",
        false,
        {
          loginHref: buildLoginHref(),
          loginLabel: "ログインする",
