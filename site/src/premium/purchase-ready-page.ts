(function attachPurchaseReadyPage(): void {
  ...
  loginPlaceholderButton?.addEventListener("click", () => {
    showMessage(
      ...
      "ログイン画面へ進みます。購入前チェックの続きや、購入済み状態の確認はログイン後にそのまま続けられます。",
    );
  ...
  loginFlowButton?.addEventListener("click", () => {
    showMessage(
      loginFlowMessage,
      "流れは『無料版を試す -> 購入前チェックでログイン -> 購入または購入済み確認 -> プレミアム版の機能を解放』です。ログイン後は購入前チェックページかプレミアム版案内ページへ戻れます。",
    );
  ...
  startCheckoutButton?.addEventListener("click", async () => {
    if (!authClient?.startCheckout) {
      showMessage(
        ...
        "この環境では購入手続きの再現がまだ使えません。接続状態を確認してから、もう一度お試しください。",
      );
      return;
    }
  ...
  simulatePurchaseButton?.addEventListener("click", async () => {
    if (!authClient?.completeCheckout) {
      showMessage(
        ...
        "この環境では購入後状態の再現がまだ使えません。接続状態を確認してから、もう一度お試しください。",
      );
      return;
    }
  ...
  purchaseStateButton?.addEventListener("click", () => {
    showMessage(
      ...
      "購入状態は『未購入 -> 購入中 -> 購入成功 -> ライセンス反映済み』の4段で考えます。決済完了直後は、短時間だけ反映確認中の表示が出ることがあります。",
    );
